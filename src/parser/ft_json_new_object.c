/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_json_new_object.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hmartzol <hmartzol@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/17 17:43:24 by hmartzol          #+#    #+#             */
/*   Updated: 2017/04/14 16:41:31 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <libftjson.h>
#include <libft.h>

inline static void			sf_jump_space(const char *src,
							const unsigned long l, unsigned long *pos)
{
	while (ft_isspace(src[*pos]) && *pos < l)
		++*pos;
}

inline static t_json_object	*sf_json_new_object(const char *src,
		const unsigned long length, unsigned long *pos, t_json_value *out)
{
	t_json_object	*obj;
	unsigned long	i;

	if ((obj = (t_json_object*)ft_memalloc(sizeof(t_json_object))) == NULL)
		return (NULL);
	obj->nb_pairs = ft_json_evaluate_object_size(src, length, (*pos)++);
	if (obj->nb_pairs < 1)
		return (obj);
	if ((obj->pair = (t_json_pair**)ft_memalloc(sizeof(t_json_pair*) *
			obj->nb_pairs)) == NULL)
		return (ft_json_free_object(&obj));
	i = 0;
	while (i < obj->nb_pairs)
	{
		obj->pair[i++] = ft_json_new_pair(src, length, pos, out);
		sf_jump_space(src, length, pos);
		if (i < obj->nb_pairs)
		{
			if (src[*pos] == ',' && *pos < length)
				++*pos;
			else
				return (ft_json_free_object(&obj));
		}
	}
	return (obj);
}

t_json_value				*ft_json_new_object(const char *src,
		const unsigned long length, unsigned long *pos, t_json_value *parent)
{
	t_json_value	*out;

	if (*pos >= length || src == NULL || src[*pos] != '{' ||
			(out = ft_json_pack_value(parent, object, NULL)) == NULL)
		return (NULL);
	out->error =
	((out->ptr = (void*)sf_json_new_object(src, length, pos, out)) == NULL);
	sf_jump_space(src, length, pos);
	if (src[*pos] == '}' && *pos < length)
		++*pos;
	else
		out->error = 1;
	return (out);
}
